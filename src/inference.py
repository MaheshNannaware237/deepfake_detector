...
def ensure_checkpoint(model_name: str) -> str:
    """
    Returns the local path to the checkpoint for `model_name`, downloading
    it from config.CHECKPOINT_URLS first if it isn't already on disk.

    This is what makes the Streamlit Cloud deployment work even though
    .pt files aren't committed to git: the first time a model is used,
    this fetches it from the GitHub Release and caches it locally for
    the rest of that session/container.
    """
    ...
    url = config.CHECKPOINT_URLS.get(model_name)
    if not url:
        raise FileNotFoundError(
            f"No local checkpoint for '{model_name}' and no download URL "
            f"configured in config.CHECKPOINT_URLS."
        )
    ...
    try:
        ...
        os.replace(tmp_path, ckpt_path)  # atomic-ish rename once fully downloaded
...
def available_checkpoints():
    """
    Returns list of model names that are usable — either already on disk,
    or downloadable from config.CHECKPOINT_URLS (e.g. on a fresh Streamlit
    Cloud deployment where nothing has been downloaded yet).
    """
    available = []
    for name in ["baseline", "transfer", "fft"]:
        local_path = os.path.join(config.CHECKPOINT_DIR, f"{name}_best.pt")
        if os.path.exists(local_path) or name in config.CHECKPOINT_URLS:
...
def load_model_cached(model_name: str):
    ...
    ckpt_path = ensure_checkpoint(model_name)
